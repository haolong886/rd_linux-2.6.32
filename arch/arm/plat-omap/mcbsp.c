...
//-----------------------haolong-------------------------
void mcbsp_clrbit_reg(unsigned int id, u16 reg, u32 bit)
{
	struct omap_mcbsp *mcbsp;
	void __iomem *io_base;
	u32 reg_data_original, reg_data;
	...
	reg_data_original = omap_mcbsp_read(io_base, reg);
	reg_data = reg_data_original & (~bit);
//#if 1
	printk(KERN_NOTICE "Set register(%x) to %x\n", reg, reg_data);
//#endif
	omap_mcbsp_write(io_base, reg, reg_data);
}
EXPORT_SYMBOL(mcbsp_clrbit_reg);
//------------------------------------------------------
void mcbsp_setbit_reg(unsigned int id, u16 reg, u32 bit)
{
	struct omap_mcbsp *mcbsp;
	void __iomem *io_base;
	u32 reg_data_original, reg_data;
...
EXPORT_SYMBOL(mcbsp_setbit_reg);
