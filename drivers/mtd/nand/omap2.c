...
//modified by haolong to use haming code ecc instead of bch ecc, 20120510
...
//	return &nand_oob;
//};
//adjust ecc layout to uboot ecc layout
struct nand_ecclayout *get_ecc_layout(int bus_width)
{
	int offset, ecc_romcode = 0;
	int i;
	...
	offset = (bus_width == 0) ? 1 : 2;
	nand_oob.oobfree->offset =
		(ecc_romcode ?  offset : (nand_oob.eccbytes + offset)); //modified
	nand_oob.oobfree->length = 64 - (nand_oob.eccbytes + offset);
